...
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../Deco")))
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../Deco/llava")))
from Deco.llava.model.builder import load_pretrained_model
from Deco.llava.conversation import conv_templates,SeparatorStyle
from Deco.llava.mm_utils import (
    process_images,
    tokenizer_image_token,
    get_model_name_from_path,
    KeywordsStoppingCriteria
)
from Deco.llava.constants import (
    IMAGE_TOKEN_INDEX,
    DEFAULT_IMAGE_TOKEN,
...
def MME_eval():
    ...
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mme_deco.pkl')
...
def POPE_coco_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_coco_deco.pkl')
...
def POPE_aokvqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_aokvqa_deco.pkl')
...
def POPE_gqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_gqa_deco.pkl')
...
def MMVet_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mmvet_deco.pkl')
    with open(f'{RESULT_PATH}/mmvet_deco.json','w') as f:
...
def MMMU_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_mmmu_deco.json', 'w') as f:
...
def CHAIR_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_chair_deco.json', 'w') as f:
...
def LLAVABENCH_eval():
    ...
    answers_file = os.path.expanduser(f"{RESULT_PATH}/eval_res_llavabench_deco.jsonl")
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description="DeCO Llama evaluation")
    parser.add_argument("--benchmark_name", type=str,default="CHAIR", help="benchmark name")

...
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../DAMO")))
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../DAMO/LLaVA")))
from DAMO.LLaVA.llava.model.builder import load_pretrained_model
from DAMO.LLaVA.llava.conversation import conv_templates
from DAMO.LLaVA.llava.mm_utils import (
    process_images,
    tokenizer_image_token,
    get_model_name_from_path,
)
from DAMO.LLaVA.llava.constants import (
    IMAGE_TOKEN_INDEX,
    DEFAULT_IMAGE_TOKEN,
...
def MME_eval():
    ...
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mme_damo.pkl')
...
def POPE_coco_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_coco_damo.pkl')
...
def POPE_aokvqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_aokvqa_damo.pkl')
...
def POPE_gqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_gqa_damo.pkl')
...
def MMVet_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mmvet_damo.pkl')
    with open(f'{RESULT_PATH}/mmvet_damo.json','w') as f:
...
def MMMU_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_mmmu_damo.json', 'w') as f:
...
def CHAIR_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_chair_damo.json', 'w') as f:
...
def LLAVABENCH_eval():
    ...
    answers_file = os.path.expanduser(f"{RESULT_PATH}/eval_res_llavabench_damo.jsonl")
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description="DAMO Llama evaluation")
    parser.add_argument("--benchmark_name", type=str,default="MMVet", help="benchmark name")

...
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../VCD")))
sys.path.insert(0, os.path.abspath(os.path.join(__file__, "../../VCD/experiments/llava")))
from VCD.experiments.llava.model.builder import load_pretrained_model
from VCD.experiments.llava.utils import disable_torch_init
...
from VCD.experiments.llava.conversation import conv_templates,SeparatorStyle
from VCD.experiments.llava.mm_utils import (
    process_images,
    tokenizer_image_token,
    get_model_name_from_path,
    KeywordsStoppingCriteria
)
from VCD.experiments.llava.constants import (
    IMAGE_TOKEN_INDEX,
    DEFAULT_IMAGE_TOKEN,
...
def MME_eval():
    ...
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mme_vcd.pkl')
...
def POPE_coco_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_coco_vcd.pkl')
...
def POPE_aokvqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_aokvqa_vcd.pkl')
...
def POPE_gqa_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_POPE_qa_gqa_vcd.pkl')
...
def MMVet_eval():
    ...
    res_df = pd.DataFrame(res)
    res_df.to_pickle(f'{RESULT_PATH}/eval_res_mmvet_vcd.pkl')
    with open(f'{RESULT_PATH}/mmvet_vcd.json','w') as f:
...
def MMMU_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_mmmu_vcd.json', 'w') as f:
...
def CHAIR_eval():
    ...
    with open(f'{RESULT_PATH}/eval_res_chair_vcd.json', 'w') as f:
...
def LLAVABENCH_eval():
    ...
    answers_file = os.path.expanduser(f"{RESULT_PATH}/eval_res_llavabench_vcd.jsonl")
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description="VCD Llama evaluation")
